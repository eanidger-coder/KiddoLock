...
app.get("/admin", async (c) => {
  ...
  const { results: devices } = await c.env.DB.prepare(
    `SELECT device_id, registered_at, last_heartbeat FROM devices ORDER BY last_heartbeat DESC LIMIT 50`
  ).all();
  const rows = devices.map((d) => `
    <tr>
      ...
      <td>
        ...
        |
        <a href="/admin/disable-kids/${d.device_id}?key=${ADMIN_KEY}" style="color:#ffa502">\u23F8 Disable Kids Mode</a>
        |
        <a href="/admin/uninstall/${d.device_id}?key=${ADMIN_KEY}" style="color:#cc0000">\u26A0\uFE0F Force Uninstall</a>
      </td>
    </tr>`).join("");
...
<h2>\u{1F4F1} Registered Devices (${devices.length})</h2>
<table><thead><tr><th>Device ID</th><th>Last Heartbeat</th><th>Actions</th></tr></thead>
