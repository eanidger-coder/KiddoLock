...
var app = new Hono2();
app.use("*", cors());
async function verifySignature(timestamp, path, signature, secret) {
...
var signatureMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (isNaN(reqTime) || Math.abs(now - reqTime) > 5 * 60 * 1e3) {
...
var ADMIN_KEY = "eitan_kiddo_master_2026";
var checkAdmin = /* @__PURE__ */ __name((c) => {
  const key = c.req.query("key") || c.req.header("X-Admin-Key");
  if (key !== ADMIN_KEY) {
    return c.text("\u{1F512} Unauthorized", 401);
  }
  return null;
}, "checkAdmin");
app.get("/admin/rename/:id", async (c) => {
  const auth = checkAdmin(c);
  if (auth) return auth;
...
app.get("/admin/feedback", async (c) => {
  ...
  try {
    ...
    const html = `<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="utf-8">
      ...
      <h1>\u{1F4DD} \u05E4\u05D9\u05D3\u05D1\u05E7 \u05DE\u05D4\u05D5\u05E8\u05D9\u05DD (${rows.length} \u05E1\u05D4"\u05DB)</h1>
      ${rows.length === 0 ? '<div class="empty">\u05E2\u05D3\u05D9\u05D9\u05DF \u05D0\u05D9\u05DF \u05E4\u05D9\u05D3\u05D1\u05E7</div>' : rows.map((r) => `
          <div class="item">
            ...
            ${r.recent_logs ? `<details><summary>\u{1F4CB} \u05DC\u05D5\u05D2\u05D9\u05DD \u05D0\u05D7\u05E8\u05D5\u05E0\u05D9\u05DD</summary><pre>${(r.recent_logs || "").replace(/</g, "&lt;")}</pre></details>` : ""}
          </div>
        `).join("")}
...
app.get("/admin", async (c) => {
  ...
  const sql = showAll ? `SELECT d.device_id, d.device_name, d.registered_at, d.last_heartbeat,
              (SELECT status_json FROM heartbeats h WHERE h.device_id = d.device_id ORDER BY h.created_at DESC LIMIT 1) as last_status
       FROM devices d ORDER BY d.last_heartbeat DESC LIMIT 200` : `SELECT d.device_id, d.device_name, d.registered_at, d.last_heartbeat,
              (SELECT status_json FROM heartbeats h WHERE h.device_id = d.device_id ORDER BY h.created_at DESC LIMIT 1) as last_status
       FROM devices d
  ...
  const rows = devices.map((d) => {
    const idShort = d.device_id.substring(0, 8);
    let fp = {};
    try {
    ...
    const ownerLine = fp.ownerEmail || fp.ownerName ? `<div style="color:#00e676;font-size:13px;margin-top:2px">\u{1F464} ${fp.ownerName || ""} ${fp.ownerEmail ? '<span style="color:#aaa">' + fp.ownerEmail + "</span>" : ""}</div>` : '<div style="color:#888;font-size:11px;margin-top:2px">\u{1F464} \u05D1\u05E2\u05DC\u05D9\u05DD \u05DC\u05D0 \u05D9\u05D3\u05D5\u05E2 (\u05D7\u05E1\u05E8\u05D4 \u05D4\u05E8\u05E9\u05D0\u05EA GET_ACCOUNTS)</div>';
    const phoneDesc = fp.manufacturer && fp.model ? `<small style="color:#aaa">\u{1F4F1} ${fp.manufacturer} ${fp.model} \xB7 Android ${fp.androidVersion || "?"} \xB7 ${fp.batteryPercent >= 0 ? fp.batteryPercent + "%\u{1F50B}" : ""}</small>` : '<small style="color:#666">\u{1F4E1} \u05DC\u05D0 \u05E7\u05D9\u05D1\u05DC heartbeat \u05E2\u05D3\u05D9\u05D9\u05DF</small>';
    const namePart = d.device_name ? `<strong style="color:#00e5ff">${d.device_name}</strong>${ownerLine}<br>${phoneDesc}<br><span style="color:#666;font-size:10px">${idShort}\u2026</span>` : `${ownerLine}<br>${phoneDesc}<br><span style="color:#888;font-family:monospace;font-size:11px">${idShort}\u2026</span>`;
    return `
    <tr>
      <td>${namePart}</td>
      ...
      <td>
        <a href="/admin/kill-device/${d.device_id}?key=${ADMIN_KEY}" style="color:#ff6b6b">\u{1F6D1} Kill</a>
        |
        ...
        |
        <a href="/admin/delete/${d.device_id}?key=${ADMIN_KEY}" style="color:#666;font-size:11px" title="\u05DE\u05D7\u05E7 \u05E8\u05E9\u05D5\u05DE\u05D4">\u{1F5D1}\uFE0F</a>
      </td>
    </tr>`;
  }).join("");
  const filterLink = showAll ? `<a href="/admin?key=${ADMIN_KEY}" style="color:#00e5ff">\u{1F4F1} \u05D4\u05E6\u05D2 \u05E8\u05E7 \u05E4\u05E2\u05D9\u05DC\u05D9\u05DD (7 \u05D9\u05DE\u05D9\u05DD \u05D0\u05D7\u05E8\u05D5\u05E0\u05D9\u05DD)</a>` : `<a href="/admin?key=${ADMIN_KEY}&show=all" style="color:#888">\u{1F5C2}\uFE0F \u05D4\u05E6\u05D2 \u05D0\u05EA \u05DB\u05DC \u05D4\u05D4\u05D9\u05E1\u05D8\u05D5\u05E8\u05D9\u05D4</a>`;
...
<h1>\u{1F6E1}\uFE0F KiddoLock Emergency Control Panel</h1>
<div class="warn">\u26A0\uFE0F \u05D4\u05DB\u05E4\u05EA\u05D5\u05E8\u05D9\u05DD \u05D4\u05D0\u05D3\u05D5\u05DE\u05D9\u05DD \u05E9\u05D5\u05DC\u05D7\u05D9\u05DD \u05E4\u05E7\u05D5\u05D3\u05D5\u05EA \u05D7\u05D9\u05E8\u05D5\u05DD \u05DC\u05DB\u05DC \u05D4\u05DE\u05DB\u05E9\u05D9\u05E8\u05D9\u05DD \u05D1\u05DE\u05E1\u05D3 \u05D4\u05E0\u05EA\u05D5\u05E0\u05D9\u05DD. \u05D4\u05E9\u05EA\u05DE\u05E9 \u05D1\u05D6\u05D4\u05D9\u05E8\u05D5\u05EA.</div>
<a class="btn kill" href="/admin/kill-all?key=${ADMIN_KEY}">\u{1F6A8} KILL ALL DEVICES (Emergency Bypass + Open App)</a>
...
<h2>\u{1F4F1} \u05DE\u05DB\u05E9\u05D9\u05E8\u05D9\u05DD ${titleSuffix} (${devices.length})</h2>
<p>${filterLink}</p>
<table><thead><tr><th>\u{1F4F1} \u05E9\u05DD / Device</th><th>\u05E4\u05E2\u05D9\u05DC\u05D5\u05EA \u05D0\u05D7\u05E8\u05D5\u05E0\u05D4</th><th>\u05E9\u05DD \u05D9\u05D3\u05D9\u05D3\u05D5\u05EA\u05D9</th><th>\u05E4\u05E2\u05D5\u05DC\u05D5\u05EA</th></tr></thead>
<tbody>${rows}</tbody></table>
<p style="margin-top:30px;color:#888;font-size:12px">\u{1F504} \u05E8\u05E2\u05E0\u05DF: <a href="/admin?key=${ADMIN_KEY}" style="color:#00e5ff">/admin</a></p>
</body></html>`;
  return c.html(html);
});
async function queueCommand(c, deviceId, commandType) {
  if (deviceId) {
    await c.env.DB.prepare(
      `INSERT INTO commands (device_id, command_type, payload) VALUES (?, ?, '')`
...
__name(queueCommand, "queueCommand");
app.get("/admin/kill-device/:id", async (c) => {
  ...
  const isFresh = dev?.last_heartbeat && new Date(dev.last_heartbeat).getTime() > Date.now() - 30 * 60 * 1e3;
  const liveBadge = isFresh ? '<div style="background:#00c853;padding:8px 12px;border-radius:6px;margin:10px 0">\u2705 \u05D4\u05DE\u05DB\u05E9\u05D9\u05E8 \u05E4\u05E2\u05D9\u05DC - \u05D4\u05E4\u05E7\u05D5\u05D3\u05D4 \u05EA\u05EA\u05D1\u05E6\u05E2 \u05EA\u05D5\u05DA 15 \u05D3\u05E7\u05D5\u05EA.</div>' : '<div style="background:#FFA502;padding:8px 12px;border-radius:6px;margin:10px 0">\u26A0\uFE0F \u05D4\u05DE\u05DB\u05E9\u05D9\u05E8 \u05DC\u05D0 \u05D3\u05D9\u05D5\u05D5\u05D7 \u05DC\u05D0\u05D7\u05E8\u05D5\u05E0\u05D4. \u05D4\u05E4\u05E7\u05D5\u05D3\u05D4 \u05DE\u05DE\u05EA\u05D9\u05E0\u05D4 \u05D5\u05EA\u05EA\u05D1\u05E6\u05E2 \u05D1\u05E8\u05D2\u05E2 \u05E9\u05D4\u05D5\u05D0 \u05D9\u05D7\u05D6\u05D5\u05E8 \u05DC\u05D0\u05D9\u05E0\u05D8\u05E8\u05E0\u05D8. \u05D0\u05DD \u05D4\u05D5\u05D0 \u05D1\u05DE\u05E6\u05D1 \u05D8\u05D9\u05E1\u05D4 \u05D0\u05D5 \u05DB\u05D1\u05D5\u05D9, \u05D4\u05D9\u05D0 \u05EA\u05D7\u05DB\u05D4.</div>';
...
app.get("/admin/kill-all", async (c) => {
  ...
  return c.html(`<div style="padding:30px;background:#0d0b1a;color:#fff;font-family:Arial;direction:rtl">
    ...
    <div style="background:${freshCnt > 0 ? "#00c853" : "#FFA502"};padding:10px;border-radius:6px;margin:15px 0">
      ${freshCnt > 0 ? `\u2705 ${freshCnt} \u05DE\u05DB\u05E9\u05D9\u05E8\u05D9\u05DD \u05E4\u05E2\u05D9\u05DC\u05D9\u05DD \u05D1\u05D7\u05E6\u05D9 \u05E9\u05E2\u05D4 \u05D4\u05D0\u05D7\u05E8\u05D5\u05E0\u05D4 - \u05D9\u05E7\u05D1\u05DC\u05D5 \u05D0\u05EA \u05D4\u05E4\u05E7\u05D5\u05D3\u05D4 \u05EA\u05D5\u05DA 15 \u05D3\u05E7\u05D5\u05EA.` : "\u26A0\uFE0F \u05D0\u05D9\u05DF \u05DE\u05DB\u05E9\u05D9\u05E8\u05D9\u05DD \u05E4\u05E2\u05D9\u05DC\u05D9\u05DD \u05DB\u05E2\u05EA. \u05D4\u05E4\u05E7\u05D5\u05D3\u05D5\u05EA \u05DE\u05DE\u05EA\u05D9\u05E0\u05D5\u05EA \u05D1\u05EA\u05D5\u05E8 \u05D5\u05D9\u05EA\u05D1\u05E6\u05E2\u05D5 \u05D1\u05E8\u05D2\u05E2 \u05E9\u05DE\u05DB\u05E9\u05D9\u05E8 \u05D9\u05D7\u05D6\u05D5\u05E8 \u05DC\u05D0\u05D9\u05E0\u05D8\u05E8\u05E0\u05D8."}
    </div>
    <p style="margin-top:20px"><a href="/admin?key=${ADMIN_KEY}" style="background:#00e5ff;color:#0d0b1a;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:bold">\u05D7\u05D6\u05D5\u05E8 \u05DC\u05D3\u05E9\u05D1\u05D5\u05E8\u05D3</a></p>
  </div>`);
});
