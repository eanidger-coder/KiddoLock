...
app.get("/admin/kill-device/:id", async (c) => {
  const auth = checkAdmin(c);
  if (auth) return auth;
  const n = await queueCommand(c, c.req.param("id"), "EMERGENCY_KILL_SWITCH");
  return c.html(`<p style="font-family:Arial;padding:20px;background:#0d0b1a;color:#0f0">\u2705 KILL command queued for 1 device. <a href="/admin?key=${ADMIN_KEY}" style="color:#00e5ff">\u05D7\u05D6\u05D5\u05E8</a></p>`);
});
...
app.get("/admin/kill-all", async (c) => {
  ...
  return c.html(`<p style="font-family:Arial;padding:20px;background:#0d0b1a;color:#0f0">\u2705 KILL_ALL queued for ${n} devices. <a href="/admin?key=${ADMIN_KEY}" style="color:#00e5ff">\u05D7\u05D6\u05D5\u05E8</a></p>`);
});
