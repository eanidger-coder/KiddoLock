...
app.get("/admin", async (c) => {
  ...
  const sql = showAll ? `SELECT device_id, device_name, registered_at, last_heartbeat FROM devices ORDER BY last_heartbeat DESC LIMIT 200` : `SELECT device_id, device_name, registered_at, last_heartbeat FROM devices
       WHERE last_heartbeat >= datetime('now','-7 days') ORDER BY last_heartbeat DESC LIMIT 50`;
  const { results: devices } = await c.env.DB.prepare(sql).all();
  const rows = devices.map((d) => {
    const idShort = d.device_id.substring(0, 8);
    const namePart = d.device_name ? `<strong style="color:#00e5ff">${d.device_name}</strong> <span style="color:#888;font-size:11px">${idShort}\u2026</span>` : `<span style="color:#888;font-family:monospace;font-size:12px">${idShort}\u2026</span>`;
    return `
    <tr>
      <td>${namePart}</td>
